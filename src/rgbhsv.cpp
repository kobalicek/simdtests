...
void ahsv_from_argb_c(float* dst, const float* src, int length) {
  for (int i = length; i; i--, dst += 4, src += 4) {
    ...
    float m = minf(minf(r, g), b);
    float v = maxf(maxf(r, g), b);
    float c = v - m;
...
void ahsv_from_argb_sse2(float* dst, const float* src, int length) {
  ...
  while ((i -= 4) >= 0) {
    ...
    xY = _mm_xor_ps(xY, XMM_GET_PS(full));                 // xY <- [V==R || V!=G]
    xZ = _mm_xor_ps(xZ, XMM_GET_PS(full));                 // xZ <- [V==R || V==G]
    
    xR = _mm_and_ps(xR, xX);                               // xR <- [X!=0 ? R : 0]
    xB = _mm_and_ps(xB, xZ);                               // xB <- [Z!=0 ? B : 0]
    xG = _mm_and_ps(xG, xY);                               // xG <- [Y!=0 ? G : 0]

    xZ = _mm_andnot_ps(xZ, XMM_GET_PS(sn));                // xZ <- [sign(!Z)]
    xY = _mm_andnot_ps(xY, XMM_GET_PS(sn));                // xY <- [sign(!Y)]
    ...
    xC = _mm_mul_ps(xC, XMM_GET_PS(m6_m6_m6_m6));          // xC <- [C*6     ]
    xG = _mm_sub_ps(xG, xB);                               // xG <- [Rx+Gx+Bx]

    xH = _mm_and_ps(xX, XMM_GET_PS(m4o6_m4o6_m4o6_m4o6));  // xH <- [V==R ?0 :-4/6]
    ...
    xC = _mm_cmple_ps(XMM_GET_PS(epsilon), xC);
    xH = _mm_add_ps(xH, XMM_GET_PS(p1));                   // xH <- [V==R ? 1 : V==G ?  2/6 :10/6]

    xG = _mm_add_ps(xG, xH);

    // Normalize H to a fraction. If it's greater than or equal to 1 then 1 is subtracted 
    // to get the Hue at [0..1) domain.
    xH = _mm_cmple_ps(XMM_GET_PS(p1), xG);

    xH = _mm_and_ps(xH, XMM_GET_PS(p1));
    xS = _mm_and_ps(xS, xC);
    xG = _mm_and_ps(xG, xC);
    xG = _mm_sub_ps(xG, xH);
...
void argb_from_ahsv_sse2(float* dst, const float* src, int length) {
  ...
  while ((i -= 4) >= 0) {
    ...
    x0 = XMM_SHUFFLE_PS(h0, _MM_SHUFFLE(1, 1, 1, 3));      // x0 <- [H           |H           |H           |V          ]
    x1 = XMM_SHUFFLE_PS(h1, _MM_SHUFFLE(1, 1, 1, 3));      // x1 <- [H           |H           |H           |V          ]
    x2 = XMM_SHUFFLE_PS(h2, _MM_SHUFFLE(1, 1, 1, 3));      // x2 <- [H           |H           |H           |V          ]
    x3 = XMM_SHUFFLE_PS(h3, _MM_SHUFFLE(1, 1, 1, 3));      // x3 <- [H           |H           |H           |V          ]

    // Calculate intervals from HUE.
    x0 = _mm_sub_ps(x0, XMM_GET_PS(p4o6_p2o6_p3o6_p0));    // x0 <- [H-4/6       |H-2/6       |H-3/6       |V          ]
    x1 = _mm_sub_ps(x1, XMM_GET_PS(p4o6_p2o6_p3o6_p0));    // x1 <- [H-4/6       |H-2/6       |H-3/6       |V          ]
    x2 = _mm_sub_ps(x2, XMM_GET_PS(p4o6_p2o6_p3o6_p0));    // x2 <- [H-4/6       |H-2/6       |H-3/6       |V          ]
    x3 = _mm_sub_ps(x3, XMM_GET_PS(p4o6_p2o6_p3o6_p0));    // x3 <- [H-4/6       |H-2/6       |H-3/6       |V          ]

    x0 = _mm_and_ps(x0, XMM_GET_PS(abs));                  // x0 <- [Abs(H-4/6)  |Abs(H-2/6)  |Abs(H-3/6)  |0          ]
    x1 = _mm_and_ps(x1, XMM_GET_PS(abs));                  // x1 <- [Abs(H-4/6)  |Abs(H-2/6)  |Abs(H-3/6)  |0          ]
    x2 = _mm_and_ps(x2, XMM_GET_PS(abs));                  // x2 <- [Abs(H-4/6)  |Abs(H-2/6)  |Abs(H-3/6)  |0          ]
    x3 = _mm_and_ps(x3, XMM_GET_PS(abs));                  // x3 <- [Abs(H-4/6)  |Abs(H-2/6)  |Abs(H-3/6)  |0          ]

    x0 = _mm_mul_ps(x0, XMM_GET_PS(m6_m6_p6_p0));          // x0 <- [-Abs(H*6-4) |-Abs(H*6-2) |Abs(H*6-3)  |0          ]
    x1 = _mm_mul_ps(x1, XMM_GET_PS(m6_m6_p6_p0));          // x1 <- [-Abs(H*6-4) |-Abs(H*6-2) |Abs(H*6-3)  |0          ]
    x2 = _mm_mul_ps(x2, XMM_GET_PS(m6_m6_p6_p0));          // x2 <- [-Abs(H*6-4) |-Abs(H*6-2) |Abs(H*6-3)  |0          ]
    x3 = _mm_mul_ps(x3, XMM_GET_PS(m6_m6_p6_p0));          // x3 <- [-Abs(H*6-4) |-Abs(H*6-2) |Abs(H*6-3)  |0          ]

    x0 = _mm_add_ps(x0, XMM_GET_PS(p1_p1_m2_p0));          // x0 <- [1-Abs(H*6-4)|1-Abs(H*6-2)|Abs(H*6-3)-2|0          ]
    x1 = _mm_add_ps(x1, XMM_GET_PS(p1_p1_m2_p0));          // x1 <- [1-Abs(H*6-4)|1-Abs(H*6-2)|Abs(H*6-3)-2|0          ]
    x2 = _mm_add_ps(x2, XMM_GET_PS(p1_p1_m2_p0));          // x2 <- [1-Abs(H*6-4)|1-Abs(H*6-2)|Abs(H*6-3)-2|0          ]
    x3 = _mm_add_ps(x3, XMM_GET_PS(p1_p1_m2_p0));          // x3 <- [1-Abs(H*6-4)|1-Abs(H*6-2)|Abs(H*6-3)-2|0          ]

    // Bound intervals.
    x0 = _mm_max_ps(x0, XMM_GET_PS(m1_m1_m1_p1));
    x1 = _mm_max_ps(x1, XMM_GET_PS(m1_m1_m1_p1));
    x2 = _mm_max_ps(x2, XMM_GET_PS(m1_m1_m1_p1));
    x3 = _mm_max_ps(x3, XMM_GET_PS(m1_m1_m1_p1));

    x0 = _mm_min_ps(x0, XMM_GET_PS(p0));                   // x0 <- [(R-1)       |(G-1)       |(B-1)       |0          ]
    x1 = _mm_min_ps(x1, XMM_GET_PS(p0));                   // x1 <- [(R-1)       |(G-1)       |(B-1)       |0          ]
    x2 = _mm_min_ps(x2, XMM_GET_PS(p0));                   // x2 <- [(R-1)       |(G-1)       |(B-1)       |0          ]
    x3 = _mm_min_ps(x3, XMM_GET_PS(p0));                   // x3 <- [(R-1)       |(G-1)       |(B-1)       |0          ]

    // Prepare S/V vectors.
    a0 = XMM_SHUFFLE_PS(h0, _MM_SHUFFLE(2, 2, 2, 2));      // a0 <- [S           |S           |S           |S          ]
    a1 = XMM_SHUFFLE_PS(h1, _MM_SHUFFLE(2, 2, 2, 2));      // a1 <- [S           |S           |S           |S          ]
    h0 = XMM_SHUFFLE_PS(h0, _MM_SHUFFLE(3, 3, 3, 0));      // h0 <- [V           |V           |V           |A          ]
    h1 = XMM_SHUFFLE_PS(h1, _MM_SHUFFLE(3, 3, 3, 0));      // h1 <- [V           |V           |V           |A          ]

    // Multiply with 'S*V' and add 'V'.
    x0 = _mm_mul_ps(x0, a0);                               // x0 <- [(R-1)*S     |(G-1)*S     |(B-1)*S     |0          ]
    x1 = _mm_mul_ps(x1, a1);                               // x1 <- [(R-1)*S     |(G-1)*S     |(B-1)*S     |0          ]
    a0 = XMM_SHUFFLE_PS(h2, _MM_SHUFFLE(2, 2, 2, 2));      // a0 <- [S           |S           |S           |S          ]
    a1 = XMM_SHUFFLE_PS(h3, _MM_SHUFFLE(2, 2, 2, 2));      // a1 <- [S           |S           |S           |S          ]

    x0 = _mm_mul_ps(x0, h0);                               // x0 <- [(R-1)*S*V   |(G-1)*S*V   |(B-1)*S*V   |0          ]
    x1 = _mm_mul_ps(x1, h1);                               // x1 <- [(R-1)*S*V   |(G-1)*S*V   |(B-1)*S*V   |0          ]
    h2 = XMM_SHUFFLE_PS(h2, _MM_SHUFFLE(3, 3, 3, 0));      // h2 <- [V           |V           |V           |A          ]
    h3 = XMM_SHUFFLE_PS(h3, _MM_SHUFFLE(3, 3, 3, 0));      // h3 <- [V           |V           |V           |A          ]
